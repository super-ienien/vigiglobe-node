    ...
    switch (typeof date)
    {
        ...
        case 'string':
            ...
            if (date.startsWith(':'))
            {
                ...
                {
                    let dur = moment.duration(Number(date.substr(6)));
                    let time = moment().startOf('day').hours(dur.hours()).minutes(dur.minutes());
    ...
    switch (typeof timeFrom)
    {
        case 'string':
            if (timeFrom.startsWith(':'))
            {
                ...
                {
                    let dur = moment.duration(Number(timeFrom.substr(6)));
                    let time = moment().startOf('day').hours(dur.hours()).minutes(dur.minutes());
                    if (time.isAfter(exports.now(-30000))) time.subtract(1, 'day');
