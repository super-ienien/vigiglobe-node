...
exports.query = function (endpoint, qs)
{
    ...
    {
        if (response.statusCode != 200) throw new VigiglobeError(response.url, params, response, response.body);
        if (response.headers.hasOwnProperty('date')) serverTimeDelta = Date.parse(response.headers.date)-Date.now();
        console.log(response.headers.date);
        console.log ('api time delta : '+serverTimeDelta);
        console.log ('api time : '+exports.now());
        console.log ('server time : '+new Date().toJSON());
    ...
    switch (typeof date)
    {
        ...
        case 'string':
            ...
            if (date.startsWith(':'))
            {
                ...
                {
                    let time = moment(date.substr(7), "hh:mm:ss");
